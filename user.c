...
void InitApp(void)
{
    ...
    IOCBbits.IOCB4 = 1;     // Enable IOC (Interrrupt on change) on pin RB4
    ...
    INT1IE = 1;             // Enable interrupt pin 1: Accelerometer interrupt
    INT2IE = 1;             // Enable interrupt pin 2: Accelerometer interrupt
...
void bubbleLevel() {
        ...
        while(!PORTBbits.RB4 || Xval > 0x90) {
            ...
            if (Ysign) {
                ...
                LATA = 0x00;
                if (Yval > 0x10) LED0 = 1;
                if (Yval > 0x30) LATA = 0b10000000;
                if (Yval > 0x50) LATA = 0b11000000;
                if (Yval > 0x70) LATA = 0b11100000;
                if (Yval > 0x90) LATA = 0b11110000;
                if (Yval > 0xB0) LATA = 0b11111000;
                if (Yval > 0x100) LATA = 0b11111100;
                if (Yval > 0x180) LATA = 0b11111110;
                if (Yval > 0x200) LATA = 0b11111111;
            } else {
                Yval = (Yval - 0x10) * 0.9;
                LED0 = 0;
                LED2 = 0;
                ...
                LATC = 0x00;
                if (Yval > 0x10) LED2 = 1;
                if (Yval > 0x30) LATC = 0b00000001;
                if (Yval > 0x50) LATC = 0b00000011;
                if (Yval > 0x70) LATC = 0b00000111;
                if (Yval > 0x90) LATC = 0b00001111;
                if (Yval > 0xB0) LATC = 0b00011111;
                if (Yval > 0x100) LATC = 0b00111111;
                if (Yval > 0x180) LATC = 0b01111111;
                if (Yval > 0x200) LATC = 0b11111111;
            }
        }
...
char readRegister(unsigned char regAddress)
{
    unsigned char data;
    ...
    write_I2C_byte(MMA8452_ADDRESS<<1);	// Send module address. 0x1D for the MMZ8452Q
...
void getAccel() {
    Xmsb = readRegister(0x01);
    Xlsb = readRegister(0x02);
    Ymsb = readRegister(0x03);
    Ylsb = readRegister(0x04);
    Zmsb = readRegister(0x05);
    Zlsb = readRegister(0x06);
    if (Xmsb & 0b10000000) {Xsign = 1; Xmsb = 0xFF - Xmsb;} else {Xsign = 0;}
    Xval = ((Xmsb << 4) + (Xlsb >> 4));
    if (Ymsb & 0b10000000) {Ysign = 1; Ymsb = 0xFF - Ymsb;} else {Ysign = 0;}
    Yval = ((Ymsb << 4) + (Ylsb >> 4));
    if (Zmsb & 0b10000000) {Zsign = 1; Zmsb = 0xFF - Zmsb;} else {Zsign = 0;}
    Zval = ((Zmsb << 4) + (Zlsb >> 4));
